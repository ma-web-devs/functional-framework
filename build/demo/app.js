...
var DEV = /(localhost)|(:5000)/.test(window.location.origin),
...
hideElem(appIframe);
getRamda('R').then(setupBookingApp, error);
...
function setupBookingApp(R) {
  ...
  if (!(iframe instanceof HTMLIFrameElement)) {
    errors.push('.iframe must be an HTMLIFrameElement, not ' + (typeof iframe === 'undefined' ? 'undefined' : _typeof(iframe)));
    return arguments[0];
  }

  iframe.addEventListener('load', function () {
    setTimeout(compose(setIframeHeight, styleVisibleElem), 0, iframe);
  });
  ...
  window.addEventListener('message', function (messageData) {
    if (messageData && messageData.isTrusted && _typeof(messageData.data) === "object") {
 ...
 */
function getRamda() {
  var NS = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 'R';

  var ram = (window || global || {})[NS];

  return new Promise(function (resolve, reject) {
    if (ram && (typeof ram === 'undefined' ? 'undefined' : _typeof(ram)) === "object" && typeof ram.map === "function") {
      // Ramda is loaded
      return resolve(ram);
    }
